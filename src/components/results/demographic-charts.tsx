...
function aggregateBySegment(
  ...
  data
    ...
    .forEach((d) => {
      const existing = map.get(d.segment_key) ?? { count: d.respondent_count, totalScore: 0, dimensions: 0 }
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={tenures} margin={{ left: 10, right: 20, top: 5, bottom: 5 }}>
                ...
                />
                <Bar dataKey="avgScore" fill="#2563eb" radius={[4, 4, 0, 0]} barSize={40} name="Promedio" />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={genders} margin={{ left: 10, right: 20, top: 5, bottom: 5 }}>
                ...
                <Tooltip
                  formatter={(value) => [Number(value).toFixed(2), "Promedio"]}
                />
                <Bar dataKey="avgScore" fill="#9333ea" radius={[4, 4, 0, 0]} barSize={40} name="Promedio" />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
